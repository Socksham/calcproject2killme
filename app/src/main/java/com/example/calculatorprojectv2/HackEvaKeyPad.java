...
public class HackEvaKeyPad extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hack_eva_key_pad);

        generator = new UnlimitedNumberGenerator();
        sTProgress = getIntent().getParcelableExtra("count");

        if (sTProgress.getFGD()){
            sTProgress.fgdFalse();
        }

        Log.d("TAG", "Count: " + sTProgress.getCount());
    ...
    private void playMusic(){
        if (backgroundMusic == null){
            backgroundMusic = MediaPlayer.create(this, R.raw.death_game);
